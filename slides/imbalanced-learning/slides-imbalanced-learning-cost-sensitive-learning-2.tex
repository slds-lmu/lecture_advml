...
\begin{vbframe}{Binary Instance-specific cost Learning}
    ...
    \begin{itemize}
        ...
        \item It is straightforward to see that class-specific cost-sensitive learning is a special case of instance-specific cost learning: $\cv_i$ same for all $\xi$.
        
        \item Consider four instances with $\mathcal{Y}=\{0,1\}$, where $\cv_i[0]$ and $\cv_i[1]$ denote the instance-wise cost of classifying an example as 0 or 1 respectively, $\yi$ the true label and $w^{(i)} =|\cv_i[1] - \cv_i[0]|$ the sample-wise weight:
        \begin{center}
                ...
                \end{tabular}
        \end{center}
        \item The empirical risk minimization problem can then be formulated as: 
        \begin{equation*}
            \mathcal{R}_{emp}(\boldsymbol{\theta}) = \sum_{i=1}^{n}w^{(i)}L(\yi,f(\xi))
        \end{equation*}
        \item Notice that instances with identical costs are not considered for the optimization of $\mathcal{R}_{emp}(\boldsymbol{\theta})$ because their weights are 0.
        \end{itemize}
            
\end{vbframe}

\begin{vbframe}{Instance-specific cost learning}
    \footnotesize
    \begin{itemize}
        \footnotesize
        ...
        \item For two instances of class 2 and 3 respectively, we have that $(\xi, \yi, \cv_i)$:
          \begin{equation*}
            \begin{aligned}
                \mat{
                    \xv^{(1)} & 2 & \mat{1 & 0 & 1} \\
...
\begin{vbframe}{CSOVO with instance-specific costs}
    ...
    \begin{itemize}
        ...
        \vspace{5pt}    
        \item Consider instance-specific costs for three observations of the form $(\xi, \yi, \cv_i)$, with some label space $\mathcal{Y}=\{1, 2, \ldots, g\}$:
        \vspace{5pt}
        \begin{equation*}
            \begin{aligned}
                \mat{
