...
\\
\textcolor{blue}{\# Relabeling: Find set of classifiers for which $\xi$ is OOB. Compute $\pi_b$ by averaging over predictions of those classifiers.
Determine new label $\tilde y^{(i)}$ w.r.t. the class that minimizes the sum of the weighted costs over the $g$ classes}
\textcolor{blue}{
\For{$i=1,\ldots,n$}
% 	 \State $\tilde M \leftarrow \bigcup_{m: \xi \notin \D_m} \{m\}$
% \textcolor{blue}{\For{$j=1,\ldots,g$} 
%    \State $\pi_j(\xi)  \leftarrow \frac{1}{|\tilde M| } \sum_{m \in \tilde M}   \pi_j(\xi~|~ f_m) $
% \EndFor
%                 }
%                 \textcolor{blue}{\State $\tilde y^{(i)} \leftarrow \argmin_{i} \sum_{j=1}^g \pi_j(\xi) C(i,j) $
