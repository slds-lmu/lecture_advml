...
\begin{vbframe}{Multiclass Costs}
    \begin{itemize}
        ...
        \item For 2 $\xi$ with $y=2$ and $y = 3$: %instances of class 2 and 3 respectively, we have that 
                \begin{center}
                            \begin{tabular}{cc|cccc}\
...
\begin{vbframe}{CSOVO with instance-specific costs}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item The costs $\cv^{(i)}[j]$ and $\cv^{(i)}[k]$ can be transformed to a \textbf{sample-wise} weight $|\cv^{(i)}[j] - \cv^{(i)}[k]|$. Please refer to \href{https://proceedings.mlr.press/v39/lin14.pdf}{\beamergotobutton{Lin et al. 2014}} for details.
            \vspace{5pt}
