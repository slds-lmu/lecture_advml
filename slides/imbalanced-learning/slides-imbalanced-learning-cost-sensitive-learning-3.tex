...
\begin{vbframe}{Binary Instance-specific cost Learning}
    ...
    \begin{itemize}
        \footnotesize
        \item Assumes instance-specific costs for every observation:  $\D^{(n)} = \{(\xi, \yi, \cv^{(i)})\}_{i=1}^n$, where $(\xi, \yi, \cv^{(i)}) \in \mathbb{R}^d \times \{0,1\} \times \mathbb{R}^2$.
...
\begin{vbframe}{Multiclass Costs}
    \begin{itemize}
        ...
        \item Vanilla CSL is special case of instance specific,\\use $\cv^{(i)}$ same for all $\xi$ of the same class
...
\begin{vbframe}{CSOVO}
    ...
    \item Theoretical guarantee: test of final classifier $\leq 2\sum_{j < k}$ test cost of $f^{(j, k)}$. 
    \end{itemize}
\end{vbframe}
